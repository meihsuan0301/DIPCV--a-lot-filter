...
def nonLocalMeans_improve(img, bigWindowSize=20, smallWindowSize=5, sigma=0.5, alpha=0.6):
    ...
    paddedImagemap[padwidth:padwidth+image.shape[0], padwidth:padwidth+image.shape[1]] = image
    paddedImagemap[padwidth:padwidth+image.shape[0], 0:padwidth] = np.fliplr(image[:,0:padwidth])
    paddedImagemap[padwidth:padwidth+image.shape[0], image.shape[1]+padwidth:image.shape[1]+2*padwidth] = np.fliplr(image[:,image.shape[1]-padwidth:image.shape[1]])
    paddedImagemap[0:padwidth,:] = np.flipud(paddedImage[padwidth:2*padwidth,:])
    paddedImagemap[padwidth+image.shape[0]:2*padwidth+image.shape[0], :] =np.flipud(paddedImagemap[paddedImage.shape[0] - 2*padwidth:paddedImage.shape[0] - padwidth,:])
